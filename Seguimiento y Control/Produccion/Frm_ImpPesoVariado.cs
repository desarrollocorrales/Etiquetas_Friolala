...
namespace Seguimiento_y_Control.Produccion
{
    public partial class Frm_ImpPesoVariado : Form
    {
        ...
        private void Frm_ImpPesoVariado_Load(object sender, EventArgs e)
        {
            ...
            txbEtiqueta.Text = frmBuscarEtiqueta.EtiquetaSeleccionada.etiqueta.ToUpper();
        ...
        private void Bascula()
        {
            ...
            string sLectura = objBascula.ReadLine();
            if (sLectura.Trim() == string.Empty || sLectura.Contains("KG") != true)
            {
                MessageBox.Show("Error en las unidades de medida...");
                return;
            }

            string subsLectura = sLectura.Substring(1, 8).Trim();
            decimal peso = Convert.ToDecimal(subsLectura);
            if (peso != 0)
            {
                //***** Si el Peso Neto es diferente de "0" Imprimir el Peso en la etiqueta
                Enviar(subsLectura);
            }
        }
